...
async def test_update_api_status(client, admin_token):
    create_resp = await client.post(
        "/apis",
        json={"name": "Payments API", "base_url": "https://api.example.com", "path": "/v1/pay", "method": "POST"},
        headers=auth_headers(admin_token),
    )
    api_id = create_resp.json()["id"]

    update_resp = await client.patch(
        f"/apis/{api_id}", json={"status": "deprecated"}, headers=auth_headers(admin_token)
    )
...
async def test_delete_api(client, admin_token):
    create_resp = await client.post(
        "/apis",
        json={"name": "Temp API", "base_url": "https://api.example.com", "path": "/v1/temp", "method": "GET"},
        headers=auth_headers(admin_token),
    )
    api_id = create_resp.json()["id"]

    delete_resp = await client.delete(f"/apis/{api_id}", headers=auth_headers(admin_token))
...
async def test_create_and_list_tags(client, admin_token):
    resp = await client.post(
        "/apis/tags", json={"name": "critical", "color": "#ff0000"}, headers=auth_headers(admin_token)
    )
    assert resp.status_code == 201
