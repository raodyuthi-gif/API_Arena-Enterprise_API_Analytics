...
async def _register_api(client, token):
    resp = await client.post(
        "/apis",
        json={"name": "Cart Service", "base_url": "https://api.example.com", "path": "/v1/cart", "method": "GET"},
...
async def test_ingest_telemetry_batch(client, admin_token):
    ...
    logs = [
        {"api_id": api_id, "endpoint_path": "/v1/cart", "method": "GET", "status_code": 200, "latency_ms": 42.5},
        {"api_id": api_id, "endpoint_path": "/v1/cart", "method": "GET", "status_code": 500, "latency_ms": 900.0},
    ]
    resp = await client.post("/telemetry/ingest", json={"logs": logs}, headers=auth_headers(admin_token))
...
async def test_ingest_rejects_empty_batch(client, admin_token):
    resp = await client.post("/telemetry/ingest", json={"logs": []}, headers=auth_headers(admin_token))
...
async def test_latency_analytics_after_ingest(client, admin_token):
    api_id = await _register_api(client, admin_token)
    logs = [
        {"api_id": api_id, "endpoint_path": "/v1/cart", "method": "GET", "status_code": 200, "latency_ms": lat}
        for lat in [10, 20, 30, 40, 50]
    ]
    await client.post("/telemetry/ingest", json={"logs": logs}, headers=auth_headers(admin_token))

    resp = await client.get(
        "/analytics/latency", params={"api_id": api_id, "window": "24h"}, headers=auth_headers(admin_token)
    )
    assert resp.status_code == 200
...
async def test_latency_analytics_invalid_window(client, admin_token):
    api_id = await _register_api(client, admin_token)
    resp = await client.get(
        "/analytics/latency", params={"api_id": api_id, "window": "3y"}, headers=auth_headers(admin_token)
    )
...
async def test_error_analytics(client, admin_token):
    api_id = await _register_api(client, admin_token)
    logs = [{"api_id": api_id, "endpoint_path": "/v1/cart", "method": "GET", "status_code": 500, "latency_ms": 5}]
    await client.post("/telemetry/ingest", json={"logs": logs}, headers=auth_headers(admin_token))

    resp = await client.get(
        "/analytics/errors", params={"api_id": api_id, "window": "24h"}, headers=auth_headers(admin_token)
    )
    assert resp.status_code == 200
