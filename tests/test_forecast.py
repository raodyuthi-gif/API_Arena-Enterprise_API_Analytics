...
async def _register_api(client, token):
    resp = await client.post(
        "/apis",
        json={"name": "Search API", "base_url": "https://api.example.com", "path": "/v1/search", "method": "GET"},
...
async def test_train_and_predict_linear_model(client, admin_token, db):
    ...
    predict_resp = await client.get(
        f"/forecast/{api_id}", params={"horizon_hours": 24}, headers=auth_headers(admin_token)
    )
