...
class ForecastModel(Base):
    ...
    model_type: Mapped[ForecastModelType] = mapped_column(
        String(30), default=ForecastModelType.PROPHET
    )
    model_path: Mapped[str] = mapped_column(
        String(500), nullable=False
    )  # path to .pkl file
    training_start: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), nullable=False
    )
    training_end: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), nullable=False
    )
    ...
    results: Mapped[list["ForecastResult"]] = relationship(
        "ForecastResult", back_populates="model", cascade="all, delete-orphan"
    )
...
class ForecastResult(Base):
    ...
    model_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("forecast_models.id"), nullable=False, index=True
    )
    api_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("api_endpoints.id"), nullable=False, index=True
    )
    ...
    # Relationships
    model: Mapped["ForecastModel"] = relationship(
        "ForecastModel", back_populates="results"
    )
